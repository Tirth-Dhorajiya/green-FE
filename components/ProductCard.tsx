...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <motion.div
      ...
      className="bg-card rounded-[2.5rem] p-4 shadow-sm hover:shadow-premium-hover hover:bg-black/5 dark:hover:bg-white/5 transition-all duration-500 group border border-black/5 dark:border-white/5 flex flex-col h-full"
    >
      <div className="relative aspect-[4/5] overflow-hidden rounded-[2rem] bg-black/5 dark:bg-white/5 mb-6">
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-2 translate-x-12 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-500">
          <button className="p-3 bg-white/20 dark:bg-black/20 backdrop-blur-md text-white dark:text-foreground rounded-2xl hover:bg-primary hover:text-white transition-colors shadow-premium border border-white/10 dark:border-white/5">
            <Heart className="w-5 h-5" />
          </button>
        </div>
        {product.stock <= 0 && (
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-[2px] flex items-center justify-center p-6 text-center">
            <span className="bg-white text-gray-900 px-6 py-2 rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl">Out of Stock</span>
          </div>
        )}
      </div>
      ...
      <div className="px-2 pb-2 flex flex-col flex-grow">
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm mb-6 line-clamp-2 leading-relaxed flex-grow">
          {product.description}
        </p>
